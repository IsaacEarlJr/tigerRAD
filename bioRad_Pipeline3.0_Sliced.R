# 5/7/25
# Added regression analysis and plotting;
# Clean comments for summer exports/upload and test.
# Coleman, I.




# This script processes NEXRAD (Level II) radar data through an abbreviated pipeline:
#
#
#   PIPELINE:
#
# We extract polar volumes (pvol) [of birds aloft] from NEXRAD radar data via AWS CLI.
#
# This script bypasses plotting imported data on a Cartesian grid via an plan position indicator (PPI)
# conversion, which is typically used for radar data visualization.
# To do this,This method would call for the 'read_vpfiles' function
#
# Instead, the pvol files are converted directly into vertical profiles (vp or vpts) using the following function:
#: vp <- calculate_vp(file_in, file_out, autoconf = TRUE)
#
# Vertical profiles have fields that consist of bird densities, speedsm and flight firection a
# various altitudes around a 35km azimuthal sweeps.
#
# - The vp/vpts fields/perameter we a focus on is Migration Traffic Rate (MTR) of birds aloft.
# - We plot between 0-3 km for altitude.
# - pvol files (example_pvol.h5) are collected at 1.5 degrees of elevation.
# - independent scans/files vary temporally by radar site, but are typically 7-9 minutes apart.
#
#   [The libraries and packages for this pipeline are derived from the following]
#
#   Dokter AM, Desmet P, Spaaks JH, Van Hoey S, Veen L, Verlinden L, Nilsson C, Haase G, Leijnse H, Farnsworth A, Bouten W,
#     Shamoun-Baranes S (2019) bioRad: biological analysis and visualization of weather radar data. Ecography 42: 852-860.
#     https://doi.org/10.1111/ecog.04028


#         [The workflow is structured into clearly defined steps below]
#
#
# INDEX:
# ----------------------------------------
...
# Define time range in GMT
# Note: (09:00:00 to 18:00:00 GMT) =
#        (4:00 AM to 1:00 PM EST) or
#        (5:00 AM to 2:00 PM EDT)
# GMT is 5 hours ahead of EST (4 EDT) or
#        6 hours ahead of CST (5 CDT)
start_time <- as.numeric("000000")#GMT
end_time <- as.numeric("235959")#GMT
...
# Specify sub directory or sub directories to load processed files
# Uncomment and modify the example below as needed
# For a specific day:
#subdirs <- "2024/12/12/KDIX"

# Specify sub directories for the entire range of interest (April 23 to May 11, 2023)
